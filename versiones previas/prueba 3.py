...
catalogo_productos = {}  # Diccionario el cual va a almacenar todos los productos del catalogo
historial_cambios = []   # Lista en la cual guardaremos el historial de todas las operaciones
categorias_existentes = []  # NUEVA: Lista que almacena las categorias creadas por el usuario
...
def crear_categoria():
    ...
    nueva_categoria = input("Nombre de la categoria: ").strip()
    ...
    # Verificamos si ya existe (sin importar mayusculas/minusculas)
    if nueva_categoria.lower() in [cat.lower() for cat in categorias_existentes]:
        print(f"Error: La categoria '{nueva_categoria}' ya existe")
        return None
    
    # Agregamos la categoria a la lista
    categorias_existentes.append(nueva_categoria)
    
    # Registramos en el historial
    cambio = f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Categoria creada: {nueva_categoria}"
    historial_cambios.append(cambio)
...
def listar_categorias():
    ...
    print("="*50)
    for i, categoria in enumerate(categorias_existentes, 1):
        print(f"{i}. {categoria}")
    print("="*50)
...
def seleccionar_categoria():
    ...
    for i, categoria in enumerate(categorias_existentes, 1):
        print(f"{i}. {categoria}")
    print(f"{len(categorias_existentes) + 1}. Crear nueva categoria")
    
    opcion = input("\nSelecciona una opcion: ").strip()
    
    # Verifica si eligio crear nueva categoria o una ya existente
    if opcion == str(len(categorias_existentes) + 1):
        return crear_categoria()
    
    try:
        indice = int(opcion) - 1
        if 0 <= indice < len(categorias_existentes):
            return categorias_existentes[indice]
        else:
...
def crear_producto(codigo: str, nombre: str, precio: float, stock: int, categoria: str):
    ...
    # Creamos un mensaje con la fecha, hora y detalles de la creacion y agregamos el cambio al historial
...
def mostrar_menu():
    ...
    print("7. Gestionar categorias")  # NUEVA OPCION
    print("8. Salir")
    print("="*50)
...
def main():
    ...
    while True:
        ...
        # Si elige cualquier otra opcion
            ...
            print("Opcion no valida. Elige entre 1-8")
